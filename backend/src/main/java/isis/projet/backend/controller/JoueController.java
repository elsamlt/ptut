...
public class JoueController {
    ...
    @DeleteMapping
    public ResponseEntity<?> deleteJoue(@RequestParam("filmId") int filmId,
                                        @RequestParam("participantId") int participantId) {
        try {
            // Trouver la relation Joue par filmId et participantId
            List<Joue> joues = joueRepository.findJouesByFilmIdAndParticipantId(filmId, participantId);

            if (!joues.isEmpty()) {
                // Supprimer la relation Joue trouvée
                joueRepository.delete(joues.get(0));
                return ResponseEntity.ok("Relation Joue supprimée avec succès");
            } else {
                return ResponseEntity.notFound().build();
            }
        } catch (Exception e) {
            return ResponseEntity.badRequest().body("Erreur lors de la suppression de la relation Joue: " + e.getMessage());
        }
    }
}